...
class Page_PickList_CollectItem extends Page {
    ...
    function init() {
        ...
        if ($_GET['store_type'] != 'Site') {
            $dest_stores->addCondition('store_type', array('Warehouse', 'Regional Store', 'Regional Stock Van'));
        } else {
            $dest_stores->addCondition('store_type', array('Regional Stock Van'));
        }
        $out = array();
...
//            $m_transfer_log->set('system_comment', $orig_qty . ' Item(s) collected from Transit Store '.$f->getElement('sel_store')->get().' to Store '.$f->getElement('Destination_Store')->get().' - TN :' . $f->getElement('tn_code')->get());
