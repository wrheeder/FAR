...
//        $f->addField('Line', 'delivery_note')->set($_GET['delivery_note']);
        ...
        $f->getElement('locators')->js(true)->closest('.atk-form-row-line')->hide();
        ...
        $serial->addHook('validate', function() use ($f, $serial, $part_catalogue_id) {
                    ...
                    if ($pc->get('serialized')) {
                        if ($serial->get() == null) {
                            ...
                            $item->tryLoadAny();
                            if (!$item->loaded()) {
                                $serial->displayFieldError('Item with this serial does not exist in your store');
                            }
                        }
                    } else {
                        if ($serial->get() != null)
        ...
        if ($f->isSubmitted()) {
            ...
            $item_id = $f->model->setStore($f->getElement('sel_store')->get(), $f->getElement('parts_catalogue_id')->get(), $f->getElement('serial')->get(), $f->getElement('Destination_Store')->get(),$f->getElement('qty')->get(),$f->getElement('tn_code')->get(),$f->getElement('locators')->get());
            //$f->update();
...
//            $m_transfer_log->saveAndUnload();
        ...
        $js = array();
        $js[] = $pc_info->js()->reload(array('Destination_Store' => $f->getElement('Destination_Store')->get(),'part_num' => $f->getElement('parts_catalogue_id')->js()->val(), 'serial_fld' => $f->getElement('serial'), 'qty_fld' => $f->getElement('qty')));
        $this->js('myFunc', $js);
    }
